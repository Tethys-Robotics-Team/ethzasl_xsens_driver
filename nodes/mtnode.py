    ...
    def spin_once(self):
        ...
        def publish_time_ref(secs, nsecs, source):
            ...
            if self.time_ref_pub is None:
                self.time_ref_pub = rospy.Publisher(
                    'imu/time_reference', TimeReference, queue_size=10)
        ...
        if self.pub_pos_gps:
            ...
            if self.pos_gps_pub is None:
                self.pos_gps_pub = rospy.Publisher('imu/fix', NavSatFix, queue_size=10)
            self.pos_gps_pub.publish(self.pos_gps_msg)
        if self.pub_vel:
            self.vel_msg.header = self.h
            if self.vel_pub is None:
                self.vel_pub = rospy.Publisher('imu/velocity', TwistStamped,
                                               queue_size=10)
